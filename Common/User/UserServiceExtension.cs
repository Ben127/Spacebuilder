...
namespace Spacebuilder.Common
{
    ...
    public static class UserServiceExtension
    {
        ...
        public static long dateAvatar = DateTime.UtcNow.Ticks;
        ...
        public static void CropAvatar(this IUserService userService, long userId, float srcWidth, float srcHeight, float srcX, float srcY)
        {
            ...
            user.Avatar = avatarRelativePath + "/" + dateAvatar+"_" + userId;

            IUserRepository userRepository = userService.GetUserRepository();
            userRepository.UpdateAvatar(user, dateAvatar, user.Avatar);
            ...
            using (Stream fileStream = iStoreFile.OpenReadStream())
            {
                ...
                storeProvider.AddOrUpdateFile(GetAvatarRelativePath(userId), GetAvatarFileNameForEdit(userId, AvatarSizeType.Big, dateAvatar), bigImage);
                storeProvider.AddOrUpdateFile(GetAvatarRelativePath(userId), GetAvatarFileNameForEdit(userId, AvatarSizeType.Small, dateAvatar) , smallImage);
        ...
        private static string GetAvatarFileNameForEdit(long userId, AvatarSizeType avatarSizeType, long dateAvatar)
        {
            string filename;
            switch (avatarSizeType)
            {
