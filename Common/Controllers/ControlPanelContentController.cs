...
namespace Spacebuilder.Common
{
    ...
    public class ControlPanelContentController : Controller
    {
        ...
        /// 更新审核状态
        /// </summary>
        /// <param name="eventIds">被操作项的Id集合</param>
        /// <param name="isApproved">是否通过审核</param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult _UpdateAuditStatus(List<long> voteIds, bool isApproved)
        {
            if (voteIds == null || voteIds.Count <= 0)
                return Json(new StatusMessageData(StatusMessageType.Error, "没有找到需要更新审核状态的投票！"));
            if (UserContext.CurrentUser.IsSuperAdministrator())
            {
            foreach (var itemId in voteIds)
            {
                var item = voteService.GetVoteThread(itemId);
                if (item == null)
                    continue;             
                    voteService.UpdateAuditStatus(itemId, isApproved);                         
            }
            return Json(new StatusMessageData(StatusMessageType.Success, "更新审核状态成功！"));     
            }
            return Json(new StatusMessageData(StatusMessageType.Error, "没有权限更新审核状态！"));
        }
        ...
        /// <param name="eventIds">投票id列表</param>
        [HttpPost]
        public JsonResult _DeleteVoteThreads(List<long> voteIds,string spaceKey=null )
        {
            if (voteIds == null || voteIds.Count <= 0)
                return Json(new StatusMessageData(StatusMessageType.Error, "没有找到需要删除的投票"));
            foreach (var itemId in voteIds)
            {
                var item = voteService.GetVoteThread(itemId);
                if (item == null)
                    continue;
                if (UserContext.CurrentUser.IsSuperAdministrator() || item.UserId == UserContext.CurrentUser.UserId)
                {
                    voteService.DeleteVoteById(itemId);
                }
                else
                {
                    return Json(new StatusMessageData(StatusMessageType.Error, "您没有删除此投票的权限"));
                }
            }
            return Json(new StatusMessageData(StatusMessageType.Success, "删除投票成功！"));
        }
