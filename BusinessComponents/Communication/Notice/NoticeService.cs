...
namespace Tunynet.Common
{
    ...
    public class NoticeService
    {
        ...
        {
            return noticeRepository.GetUnhandledCountNoComment(userId);
        }
        ...
        /// <param name="pageIndex">页码</param>
        /// <returns>通知分页集合</returns>
        public PagingDataSet<Notice> GetsForNoComment(long userId, NoticeStatus? status, int? typeId, int? applicationId, int? pageIndex = null)
        {
            //按照创建日期倒序排序
            return noticeRepository.GetsForNoComment(userId, status, typeId, applicationId, pageIndex ?? 1);
        }
        /// <summary>
        /// 获取所有未处理的通知
        /// </summary>
