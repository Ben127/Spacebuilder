...
namespace Spacebuilder.Blog
{
    ...
    public class BlogThreadRepository : Repository<BlogThread>, IBlogThreadRepository
    {
        ...
        /// <param name="threadId">日志id</param>
        /// <returns>解析后的日志正文</returns>
        public string GetResolvedBodyForMobile(long threadId)
        {
            string cacheKey = GetCacheKeyOfResolvedBodyForMobile(threadId);
            string resolveBody = cacheService.Get<string>(cacheKey);
            ...
            if (resolveBody == null)
            {
                ...
                {
                    BlogThread blogThread = Get(threadId);
                    resolveBody =new BlogBodyProcessor().ProcessForMobile(resolveBody, TenantTypeIds.Instance().BlogThread(), threadId, blogThread.UserId);
                    cacheService.Set(cacheKey, resolveBody, CachingExpirationType.SingleObject);
                }
            }
        ...
        {
            return "BlogThreadResolvedBodyForMobile" + threadId;
        }
        /// <summary>
        /// 可对外显示的审核状态
        /// </summary>
