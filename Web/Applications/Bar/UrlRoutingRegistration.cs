...
namespace Spacebuilder.Bar
{
    public class UrlRoutingRegistration : AreaRegistration
    {
        ...
        public override void RegisterArea(AreaRegistrationContext context)
        {
            ...
            context.MapRoute(
                "Event_Bar_ThreadDetail", // Route name
                "Event/bar/t-{threadId}" + extensionForOldIIS, // URL with parameters
                new { controller = "EventSpaceBar", action = "Detail" }, // Parameter defaults
                new { threadId = @"(\d+)|(\{\d+\})" } // Parameter defaults
            );

            context.MapRoute(
                "Event_Bar_Edit", // Route name
                "Event/bar/Edit" + extensionForOldIIS, // URL with parameters
                new { controller = "EventSpaceBar", action = "Edit" }
            );
            context.MapRoute(
                "Event_Bar_Common", // Route name
                "Eventbar/{action}" + extensionForOldIIS, // URL with parameters
                new { controller = "EventSpaceBar", action = "Detail" } // Parameter defaults
            );
