...
namespace Spacebuilder.Microblog
{
    ...
    public class MicroblogRepository : Repository<MicroblogEntity>, IMicroblogRepository
    {
     ...
     /// <summary>
        ...
        /// <param name="microblogId">微博Id</param>
        /// <returns></returns>
        public string GetResolvedBodyForMobile(long microblogId)
        {
            MicroblogEntity microblog = Get(microblogId);
            if (microblog == null)
                return string.Empty;

            string cacheKey = string.Format("GetResolvedBodyForMobile{0}::{1}", RealTimeCacheHelper.GetEntityVersion(microblogId), microblogId);
            string resolveBody = cacheService.Get<string>(cacheKey);
            if (string.IsNullOrEmpty(resolveBody))
            {
                resolveBody = microblog.Body;
                IMicroblogBodyProcessor microblogBodyProcessor = DIContainer.Resolve<IMicroblogBodyProcessor>();
                resolveBody = microblogBodyProcessor.ProcessForMobile(resolveBody, microblog.TenantTypeId, microblog.MicroblogId, microblog.UserId, microblog.OwnerId);
                cacheService.Set(cacheKey, resolveBody, CachingExpirationType.UsualSingleObject);
            }
            return resolveBody;
        }
        ...
        {
            Sql sql = Sql.Builder;
            sql.InnerJoin("tn_Counts_100101").On("spb_Microblogs.MicroblogId=tn_Counts_100101.ObjectId");
            if (ownerId.HasValue)
