...
namespace Spacebuilder.Common
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            //ViewData["EnablePhone"] = ConfigurationManager.AppSettings["EnablePhone"];
            //ViewData["RequiredPhone"] = ConfigurationManager.AppSettings["RequiredPhone"];
        ...
        public ActionResult Register(RegisterEditModel model)
        {
            ...
            //根据站点配置设置用户属性
            user.IsActivated = userSettings.AccountActivation == AccountActivation.Email;
        ...
        private ActionResult RegisterJumpByconfig(IUser user)
        {
            ...
            try
            {
                ...
                //异步发送
                string errorMessage = "";
                sendEmailSuccess = emailService.Send(mailMessage, out errorMessage);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Login(LoginEditModel model)
        {
            ...
            if (userLoginStatus == UserLoginStatus.Success)
            {
                ...
                {
                    return Redirect(Tunynet.Utilities.WebUtility.UrlDecode(model.ReturnUrl.Replace("[%]","%")));
                }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult PhoneVerification(string phone)
        {
            long telePhone = Int64.Parse(phone);
            string verification = Page_Load(telePhone);
            return Content(verification.ToString());
        }
        protected string Page_Load(long phone)
        {
            string ret = null;
            DateTime datetime = DateTime.Now;
            string Dtime = datetime.ToString("yyyy-MM-dd");
            Random ran = new Random();
            string verification= System.Guid.NewGuid().ToString().Substring(0, 6);   
            //int verification = ran.Next(100000, 999999);//生成验证码
            Session["verification"] = verification;
            CCPRestSDK.CCPRestSDK api = new CCPRestSDK.CCPRestSDK();
            //ip格式如下，不带https://
            bool isInit = api.init("app.cloopen.com", "8883");
            api.setAccount("8a48b5514e1f2cc0014e1f6d2b1b008c", "74ea1d760e8f473ca9f218440b4ccd91");
            api.setAppId("8a48b5514e236232014e2458322c0178");
            ...
            try
            {
                ...
                {
                    int minutes = 15;
                    string[] dates = { verification.ToString(), minutes.ToString() };//验证码,时间
                    Dictionary<string, object> retData = api.SendTemplateSMS(phone.ToString(), "29292", dates);//短信接收号码,短信模板id,内容数据
